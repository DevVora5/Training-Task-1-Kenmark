...
// Exported deleteComment validation middleware
exports.deleteCommentValidator = [
    ...
    async (req, res, next) => {
        try {
            ...
            // Find the associated post for the comment
            const post = await Post.findByPk(comment.postId);

            // Check if the post exists
            if (!post) {
                return res.status(404).json({ errors: [{ msg: "Post not found for the comment" }] });
            }

            // Check if the user is the post author or the comment author
            if (userId !== post.authorId && userId !== comment.userId) {
                return res.status(403).json({ errors: [{ msg: "Unauthorized to delete comment" }] });
            }
